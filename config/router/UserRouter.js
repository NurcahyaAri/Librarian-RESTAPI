...
module.exports = async (fastify, opts, next) => {
    ...
    fastify.get("/", async (request, reply) => {
        if(Auth.checkAuth(request, reply).isAdmin()){
    ...
    fastify.get("/profile/", async (request, reply) => {
        if(Auth.checkAuth(request, reply).state.accepted){
    ...
    fastify.put('/:id', async (request, reply) => {
        ...
        if(Auth.checkAuth(request, reply).isAdmin() || userData.sub === request.params.id){
